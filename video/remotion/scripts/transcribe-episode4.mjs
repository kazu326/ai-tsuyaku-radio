...
// Episode 4 only: use the supplied narration files as the source of truth.
...
const only = onlyArgument ? Number(onlyArgument.slice("--only=".length)) : null;
if (only !== null && (!Number.isInteger(only) || only < 1 || only > 19)) {
  throw new Error("--only must be an integer from 1 to 19");
}
...
for (let number = 1; number <= 19; number += 1) {
  ...
  const audioPath = `public/episode4/${name}`;
  const output = `docs/episode4/stt-${id}.json`;
  ...
  await fs.mkdir("docs/episode4", {recursive: true});
